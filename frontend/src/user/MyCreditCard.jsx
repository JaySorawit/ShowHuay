 ...
 *   MyCreditCard.jsx                                               *
 *                                                                  *
 *   This page is presenting user's credit card information.        *
 *   It usually includes options for managing credit cards, such as *
 *   viewing, adding, editing, and deleting cards on the user's     *
 *   account.                                                       *
 *                                                                  *
 ********************************************************************/
...
function MyCreditCard() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const creditCardResponse = await fetch(
        `http://localhost:3000/creditCard/addCreditCard/${userId}`,
        {
          method: "POST",
          headers: {
  ...
  const handleEdit = async (creditCardId) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:3000/creditCard/getOneCreditCard/${creditCardId}`
      );
      if (response.ok) {
  ...
  const handlecreditCardCheck = async () => {
    try {
      const response = await fetch(
        `http://localhost:3000/creditCard/check-creditCard?creditCardNumber=${formData.creditCardNumber}`
      );
      const data = await response.json();
      ...
      setcreditCardExists(
        data.exists && formData.creditCardNumber !== creditCard.creditCardNumber
      );
    } catch (error) {
  ...
  const handlecreditCardEditCheck = async () => {
    try {
      const response = await fetch(
        `http://localhost:3000/creditCard/check-creditCard?creditCardNumber=${creditCard.creditCardNumber}`
      );
      const data = await response.json();
      console.log("creditCard check response:", data);
      console.log("creditCard check response:", data.exists);
      ...
      if (
        data.exists === true &&
        creditCard.creditCardNumber !== formData.creditCardNumber
      ) {
        setcreditCardExists(true);
      } else if (
        data.exists === false &&
  ...
  const handleUpdatecreditCard = async (e) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/creditCard/updatecreditCard/${formData.creditCardId}"`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
                        ...
                        >
                          {creditCard.card_default === 0 && (
                            <React.Fragment>
                              <p
                                onClick={() =>
                                  handleEdit(creditCard.credit_card_id)
                                }
                                style={{
                                  color: "#F44C0C",
                                  cursor: "pointer",
                              ...
                              <p
                                onClick={() =>
                                  handleDelete(creditCard.credit_card_id)
                                }
              ...
              {ModalOpen && (
                <div className="modal">
                  <div className="modal-content">
                    <span
                      className="close"
                      onClick={() => toggleAddcreditCardModal()}
                    >
                      &times;
                    </span>
                    <h5
              ...
              {modalEdit && (
                <div className="modal">
                  <div className="modal-content">
                    <span
                      className="close"
                      onClick={() => toggleEditcreditCardModal()}
                    >
                      &times;
                    </span>
                    <h5
